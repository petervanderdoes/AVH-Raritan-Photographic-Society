...
class MyEntriesModel
{
    ...
    private function addFormButtons(Competition $current_competition, Form $form)
    {
        ...
        $total_entries_submitted = $this->query_entries->countEntriesSubmittedByMember(get_current_user_id(),
                                                                                       $current_competition->Competition_Date);

        // Don't show the Add button if the max number of images per member reached
        if ($this->num_rows < $max_entries_per_member_per_comp &&
            $total_entries_submitted < $this->settings->get('club_max_entries_per_member_per_date')
        ) {
     ...
     * @return array
     */
    private function getEntries(Competition $current_competition)
    {
        $data    = [];
    ...
    {
        $open_competitions = $this->query_competitions->getOpenCompetitions(get_current_user_id(),
                                                                            $medium_subset_medium);
        $open_competitions = CommonHelper::arrayMsort($open_competitions,
     ...
     * @return array
     */
    private function getTemplateData(Competition $current_competition)
    {
        $data                     = [];
        ...
        if ($close_date !== null) {
            ...
            $close_competition_warning_date = Carbon::instance(new \DateTime($close_date,
     ...
     * @param Competition $current_competition
     */
    private function saveSession($medium_subset_medium, Competition $current_competition)
    {
