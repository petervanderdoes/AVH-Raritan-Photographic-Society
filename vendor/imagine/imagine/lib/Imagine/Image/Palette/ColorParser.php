...
class ColorParser
{
    ...
    public function parseToGrayscale($color)
    {
        ...
        if (1 !== count($color)) {
            throw new InvalidArgumentException(
                'The provided color has different values of red, green and blue components. Grayscale colors must have the same values for these.'
            );
        }
    ...
    private function parse($color)
    {
        if (!is_string($color) && !is_array($color) && !is_int($color)) {
            throw new InvalidArgumentException(
                sprintf('Color must be specified as a hexadecimal string, array or integer, %s given', gettype($color))
            );
        }
        ...
        if (is_array($color)) {
            ...
            throw new InvalidArgumentException(
                'Color argument if array, must look like array(R, G, B), or array(C, M, Y, K) where R, G, B are the integer values between 0 and 255 for red, green and blue or cyan, magenta, yellow and black color indexes accordingly'
            );
        }
        ...
        if (is_string($color)) {
            if (0 === strpos($color, 'cmyk(')) {
                ...
                if (strlen($color) !== 3 && strlen($color) !== 6) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'Color must be a hex value in regular (6 characters) or short (3 characters) notation, "%s" given',
                            $color
                        )
                    );
                }
        ...
        if (is_int($color)) {
            $color = [255 & ($color >> 16), 255 & ($color >> 8), 255 & $color];
        }
