...
final class Image extends AbstractImage
{
    ...
    public function paste(ImageInterface $image, PointInterface $start)
    {
        if (!$image instanceof self) {
            throw new InvalidArgumentException(
                sprintf('Imagick\Image can only paste() Imagick\Image instances, %s given', get_class($image))
            );
        }
    ...
    public function pixelToColor(\ImagickPixel $pixel)
    {
        ...
        $alpha = $this->palette->supportsAlpha() ? (int) round(
            $pixel->getColorValue(\Imagick::COLOR_ALPHA) * 100
        ) : null;
    ...
    public function usePalette(PaletteInterface $palette)
    {
        if (!isset(static::$colorspaceMapping[$palette->name()])) {
            throw new InvalidArgumentException(
                sprintf('The palette %s is not supported by Imagick driver', $palette->name())
            );
        }
    ...
    private function applyImageOptions(\Imagick $image, array $options, $path)
    {
        ...
        if (isset($options['jpeg_quality']) && in_array($format, ['jpeg', 'jpg', 'pjpeg'])) {
    ...
    {
        return $fill instanceof Linear && $fill->getStart()
                                               ->isOpaque() && $fill->getEnd()
                                                                    ->isOpaque()
            ;
    }
