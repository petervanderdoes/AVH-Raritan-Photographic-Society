...
class AVH2_PluginController
{
	...
	public function __construct (AVH2_Settings $settings, AVH2_Options $options)
	{
		...
		// well.
		$this->_pluginfile = $this->_settings->getSetting('plugin_file');
		$this->_textdomain = $this->_settings->getSetting('text-domain');
		self::$base_url = $this->_settings->getSetting('plugin_url');
	}
	...
	public function actionInit ()
	{
		if ( !isset($this->_textdomain) ) {
			load_plugin_textdomain($this->_textdomain, FALSE, $this->_settings->getSetting('plugin_dir') . '/lang');
		}
		
		// Register Styles and Scripts
		
		$_style = $this->_getStyleName();
		wp_register_style($_style . '-css', $this->_settings->getSetting('plugin_url') . '/css/' . $_style . '.css', array(), $this->_settings->getSetting('plugin_version'), 'screen');
	}
	...
	protected function _getStyleName ($style = '')
	{
		...
		if ( empty($style) )
			if ( is_admin() ) {
				$_full_style_name = $this->_settings->getSetting('file_prefix') . 'admin' . $_minified;
			} else {
				$_full_style_name = $this->_settings->getSetting('file_prefix') . 'public' . $_minified;
			}
		else {
			$_full_style_name = $this->_settings->getSetting('file_prefix') . $style . $_minified;
		}
	...
	protected function _getJsName ($script = '')
	{
		...
		if ( empty($script) ) {
			if ( is_admin() ) {
				$_full_script_name = $this->_settings->getSetting('file_prefix') . 'admin' . $_minified;
			} else {
				$_full_script_name = $this->_settings->getSetting('file_prefix') . 'public' . $_minified;
			}
		} else {
			$_full_script_name = $this->_settings->getSetting('file_prefix') . $script . $_minified;
		}
	...
	{
		$_response = wp_remote_get($this->_settings->getSetting('plugin_readme_url'), array('user-agent' => 'WordPress/' . AVH2_Common::getWordpressVersion() . ' ' . $this->_settings->getSetting('plugin_name') . '/' . $this->_settings->getSetting('plugin_version')));
		if ( !is_wp_error($_response) || is_array($_response) ) {
			...
			if ( preg_match('~==\s*Changelog\s*==\s*=\s*Version\s*[0-9.]+\s*=(.*)(=\s*Version\s*[0-9.]+\s*=|$)~Uis', $_data, $_matches) ) {
				...
				if ( $_prev_version[0] != $this->_settings->getSetting('plugin_version') ) {
					echo '<div style="color: #f00; font-weight: bold;">';
					echo '<br />';
					echo sprintf(__('The installed version, %s, is more than one version behind.', $this->_textdomain), $this->_settings->getSetting('plugin_version'));
					echo '<br />';
	...
	public function add_page (array $args)
	{
		...
		// @format_off
			$_default_args = array(	 'id' 				=> '',
									 'parent_id'		=> '',
									 'type'				=> 'options',
									 'page_title'		=> $this->_settings->getSetting('plugin_name').__(' settings', $this->_textdomain),
									 'menu_title'		=> $this->_settings->getSetting('plugin_name'),
	...
	{
		foreach ( $this->_tinyMCE_buttons as $_value ) {
			$plugin_array[$_value->name] = $this->_settings->getSetting('plugin_url') . $_value->path . '/' . $_value->js_file;
		}
		return $plugin_array;
	}
